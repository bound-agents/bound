...
describe("Platform MCP Registry — Relay Integration (AC6 + AC7)", () => {
	...
	// AC7.1 legacy relay_outbox intake tests removed at release N+1: deliverBatch
	// now writes an unconditional durable_work intake row (kind connector_intake)
	// regardless of host mode. The durable intake write and its idempotency fence
	// are covered by connector-intake.test.ts.
