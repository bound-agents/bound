 ...
 * `discord`, `discord-interaction`, `scheduler`, `webhook`, etc.)
 ...
 * system-driven (cron wakeups, webhook intake) and should be invisible
 * to the operator and to cosmetic concerns like the thread-color cycle.
 *
 ...
 */
export const NON_USER_FACING_INTERFACES = ["scheduler", "webhook"] as const;

/**
 * Returns true when a thread interface tag represents a user-facing surface.
 * Returns false for `scheduler`, `webhook`, and for null/undefined/empty.
 *
 * Used by:
