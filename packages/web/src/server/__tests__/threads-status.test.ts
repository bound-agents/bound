...
// The "/api/status/cancel with delegation (AC6.4)" suite was removed: whole-loop
// `process` delegation no longer exists (single delegation path, R-UD1), so the
// cancel endpoint never propagates a relay `cancel` to a delegated host. The
// local `agent:cancel` event reaches the locally-running loop directly, and
// in-flight relayed inference is cancelled by the loop's own relay cancel path.
