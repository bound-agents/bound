...
function futureTime(nowMs: number, offsetMs: number): string {
	return new Date(nowMs + offsetMs).toISOString();
}

function pastTime(nowMs: number, offsetMs: number): string {
	return new Date(nowMs - offsetMs).toISOString();
}

const HOUR = 3_600_000;
const BASE_NOW = Date.parse("2026-08-20T03:30:00.000Z");
...
describe("rankDepartures", () => {
	...
	it("should sort active tasks (running/claimed) above non-active", () => {
		const t = futureTime(BASE_NOW, HOUR);
		const tasks: Task[] = [
	...
	it("should include a departure exactly at the captured now boundary", () => {
		const tasks: Task[] = [
			{ id: "at-boundary", status: "pending", next_run_at: new Date(BASE_NOW).toISOString() },
			{ id: "after-boundary", status: "pending", next_run_at: futureTime(BASE_NOW, 1) },
		];

		const ranked = rankDepartures(tasks, 2, BASE_NOW);

		expect(ranked.map((task) => task.id)).toEqual(["at-boundary", "after-boundary"]);
	});

	it("should sort by next_run_at ascending within same status", () => {
		const tasks: Task[] = [
			{ id: "later", status: "pending", next_run_at: futureTime(BASE_NOW, 3 * HOUR) },
			{ id: "sooner", status: "pending", next_run_at: futureTime(BASE_NOW, HOUR) },
	...
	it("should keep running tasks above pending even when pending has earlier next_run_at", () => {
		const tasks: Task[] = [
			{ id: "pending-soon", status: "pending", next_run_at: futureTime(BASE_NOW, HOUR) },
	...
	it("should exclude failed tasks that are not running or upcoming", () => {
		const tasks: Task[] = [
			{ id: "failed-past-1", status: "failed", next_run_at: pastTime(BASE_NOW, HOUR) },
			{ id: "failed-past-2", status: "failed", next_run_at: pastTime(BASE_NOW, 2 * HOUR) },
			{ id: "failed-past-3", status: "failed", next_run_at: pastTime(BASE_NOW, 3 * HOUR) },
			{ id: "failed-past-4", status: "failed", next_run_at: pastTime(BASE_NOW, 4 * HOUR) },
			{ id: "failed-past-5", status: "failed", next_run_at: pastTime(BASE_NOW, 5 * HOUR) },
			{ id: "failed-past-6", status: "failed", next_run_at: pastTime(BASE_NOW, 6 * HOUR) },
			{ id: "heartbeat", status: "pending", next_run_at: futureTime(BASE_NOW, HOUR) },
		];
	...
	it("should not mutate input array", () => {
		const tasks: Task[] = [
			{ id: "2", status: "pending", next_run_at: futureTime(BASE_NOW, 2 * HOUR) },
			{ id: "1", status: "running", next_run_at: futureTime(BASE_NOW, HOUR) },
		];
