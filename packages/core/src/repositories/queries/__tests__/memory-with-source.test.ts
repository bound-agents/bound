...
// ── countEligibleMemoryDelta (aggregate) ─────────────────────────────────────

describe("countEligibleMemoryDelta", () => {
	const BASELINE = "2026-01-01T00:00:00.000Z";

	it("counts only live, non-_internal, eligible-tier entries after baseline", () => {
		insertRow(
			db,
			"semantic_memory",
			makeMemory({
		...
		// excluded: pinned, summary
		insertRow(
			db,
			"semantic_memory",
			makeMemory({ id: "p", key: "pin", tier: "pinned", modified_at: "2026-02-03T00:00:00.000Z" }),
			SITE,
		);
		insertRow(
			db,
			"semantic_memory",
			makeMemory({ id: "s", key: "sum", tier: "summary", modified_at: "2026-02-03T00:00:00.000Z" }),
			SITE,
		);
		// excluded: _internal key
		insertRow(
			db,
			"semantic_memory",
			makeMemory({
		...
		// excluded: at-baseline (not strictly after)
		insertRow(
			db,
			"semantic_memory",
			makeMemory({ id: "b", key: "at-base", tier: "default", modified_at: BASELINE }),
			SITE,
		);

		const result = countEligibleMemoryDelta(db, BASELINE);
		expect(result).toEqual({ cnt: 2 });
	});

	it("excludes soft-deleted rows (deleted=0 filter)", () => {
		insertRow(
			db,
			"semantic_memory",
			makeMemory({
		...
		expect(countEligibleMemoryDelta(db, BASELINE)).toEqual({ cnt: 1 });
	});

	it("excludes a detail entry covered by a live summarizes edge", () => {
		insertRow(db, "semantic_memory", makeMemory({ id: "sum", key: "sum", tier: "summary" }), SITE);
		insertRow(
			db,
			"semantic_memory",
		...
		insertRow(
			db,
			"memory_edges",
			makeEdge({ id: "e1", source_key: "sum", target_key: "covered" }),
			SITE,
		);

		expect(countEligibleMemoryDelta(db, BASELINE)).toEqual({ cnt: 0 });
	});

	it("zero-row case returns a row with cnt 0 (COUNT(*) never yields null)", () => {
		expect(countEligibleMemoryDelta(db, BASELINE)).toEqual({ cnt: 0 });
	});
});
