...
- `boundless_read_structure` returns a flat, body-free declaration outline with hashline-compatible `LINE:HASH|symbolName` rows. It has one `path` parameter and is intentionally scoped to JavaScript and TypeScript-family source through the direct TypeScript compiler API: TypeScript `.ts`, TSX `.tsx`, JavaScript `.js`, `.mjs`, `.cjs`, JSX `.jsx`, and declaration files `.d.ts`, `.d.mts`, `.d.cts`. It does not extract structure from other languages. It emits top-level named functions, classes, identifier variables, interfaces, type aliases, enums, namespaces/modules, named exports, and named default functions/classes; imports, anonymous defaults, export stars, and nested declarations are omitted. Extensions are dispatched explicitly and case-insensitively: extensionless and unregistered files (including Python, Go, Rust, Svelte, configuration, document, markup, and stylesheet formats) return an empty result rather than being parsed as TypeScript. Malformed supported source is rejected when TypeScript reports parse diagnostics; parser failures return an error and never fall through to another parser. Source input is limited to 1,000,000 UTF-8 bytes before parsing; rendered output is capped at 50,000 UTF-8 bytes with no partial symbol row. Anchors hash normalized physical source lines and remain compatible with `boundless_edit`.
