...
export const cacheDiagnostic: Diagnostic = {
	name: "cache",
	...
	render(perTurnRecords: ReadonlyArray<Record<string, unknown>>): string {
		// Reset module state for repeated runs (safe even though run.ts is
		// process-bounded, but keeps tests deterministic).
		priorBody = null;

		const recs = perTurnRecords as unknown as ReadonlyArray<CacheTurnRecord>;
		if (recs.length === 0) {
