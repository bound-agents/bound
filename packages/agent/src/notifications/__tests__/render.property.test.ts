 ...
 *
 * The renderer now projects only resolved-advisory operator-acks into
 * `[Advisory notification]` lines (skill-retirement notifications were
 * removed along with the retire operation).
 *
 * Properties:
 *
 ...
 *   F6 Line-shape — every line has the [Advisory notification] tag prefix.
 *   F7 Order preservation — input order drives output order.
 *   F8 Capture-time — every ack line carries a relative-time fragment (#71).
 */

import { describe, it } from "bun:test";
import fc from "fast-check";
import { ADVISORY_NOTIF_CAP, type ResolvedAdvisoryRow, renderNotifications } from "../render";
...
describe("renderNotifications — property tests", () => {
	it("F1: determinism — same inputs produce byte-equal output", () => {
		fc.assert(
			fc.property(fc.array(advisoryArb, { maxLength: 16 }), (resolvedAdvisories) => {
				const a = renderNotifications({ resolvedAdvisories, nowMs: NOW_MS }).join("\n");
				const b = renderNotifications({ resolvedAdvisories, nowMs: NOW_MS }).join("\n");
				return a === b;
			}),
			{ numRuns: 100 },
		);
	...
	it("F2: cap — never emits more than ADVISORY_NOTIF_CAP advisory lines", () => {
		fc.assert(
			fc.property(fc.array(advisoryArb, { maxLength: 30 }), (resolvedAdvisories) => {
				const out = renderNotifications({ resolvedAdvisories, nowMs: NOW_MS });
				return out.length <= ADVISORY_NOTIF_CAP;
			}),
			{ numRuns: 100 },
		);
	});
		...
		fc.assert(
			fc.property(
				...
				(title, status, resolvedAt, count) => {
					...
					const out = renderNotifications({ resolvedAdvisories: advisories, nowMs: NOW_MS });
	...
	it("F3b: single advisory has no count marker", () => {
		fc.assert(
			fc.property(advisoryArb, (advisory) => {
				const out = renderNotifications({ resolvedAdvisories: [advisory], nowMs: NOW_MS });
	...
	it("F6: every emitted line has the [Advisory notification] tag prefix", () => {
		fc.assert(
			fc.property(fc.array(advisoryArb, { maxLength: 12 }), (resolvedAdvisories) => {
				const out = renderNotifications({ resolvedAdvisories, nowMs: NOW_MS });
				for (const line of out) {
					if (!line.startsWith("[Advisory notification]")) return false;
				}
				return true;
			}),
			{ numRuns: 100 },
	...
	it("F7: order preservation — advisory input order is kept", () => {
		fc.assert(
			fc.property(
				fc
				...
				(resolvedAdvisories) => {
					const out = renderNotifications({ resolvedAdvisories, nowMs: NOW_MS });
					for (let i = 0; i < resolvedAdvisories.length; i++) {
						if (!out[i].includes(`'${resolvedAdvisories[i].title}'`)) return false;
					}
					return true;
				},
			),
