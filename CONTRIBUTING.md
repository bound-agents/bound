...
- **Cross-provider `tool_use.id` portability**: `tool_use.id` values persist in the `messages` table and survive provider switches. Anthropic enforces `^[a-zA-Z0-9_-]+$` and rejects the entire request when a historical id contains anything else; Bedrock Converse and OpenAI-compatible providers accept a broader charset. The OpenAI-compatible path (notably the Kimi/Moonshot fallback, where the AI SDK synthesizes ids from `function.name + index` when the upstream emits no explicit id) routinely produces ids of the shape `functions.<name>:<index>` containing `.` and `:`. When such a thread later routes to opus or Bedrock-Anthropic, the API returns `messages.N.content.M.tool_use.id: String should match pattern '^[a-zA-Z0-9_-]+$'` and the turn fails. **Fix:** `sanitizeToolUseId` in `packages/llm/src/ai-sdk-bridge.ts` replaces every character outside the safe charset with `_`. The transform is deterministic and applied at all four sites in `toModelMessages` (assistant `tool_call`-role tool_use parts, inline assistant content tool_use parts, `tool_result.tool_use_id`, and the `toolNameById` index keys), so pairing between `tool_use` and `tool_result` and toolName resolution are preserved. The safe charset is a strict subset of every supported provider's accepted charset, so universal sanitization is lossless on the wire and avoids per-provider branching. When adding a new id-bearing field at the bridge boundary, route it through `sanitizeToolUseId` too.
