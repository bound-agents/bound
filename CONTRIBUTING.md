...
- **Per-stream bash output cut (`50KB`, fires before the 256 KiB cap)**: both bash surfaces middle-cut stdout and stderr *independently* through `truncateStreamOutput(stream, HALF_OUTPUT_BYTES)` (`@bound/shared/strings.ts`, `HALF_OUTPUT_BYTES = 50000`) before assembling the result — `boundless_bash` at `packages/less/src/tools/bash.ts` and the sandbox `bash` result formatter at `packages/agent/src/agent-loop-utils.ts`. This runs BEFORE the universal 256 KiB `capToolResultContent` backstop, so a command that floods a single stream is bounded at ~50KB for that stream (it's per-stream, so a 50KB stdout plus a 50KB stderr can still total ~100KB before the backstop sees it). `boundless_bash` carried this cut first; `3b088b77` brought the sandbox path in line so both surfaces truncate identically. If bash output looks middle-truncated well below the 256 KiB marker, this is why.
