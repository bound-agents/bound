...
- **@bound/client**: The existing TypeScript client library for talking to bound over HTTP + WebSocket. Used by the web UI and now boundless.
...
**BoundClient usage** (`packages/client/src/client.ts`) — boundless uses BoundClient identically to how the web UI uses it: `connect()`, `subscribe()`, `sendMessage()`, `configureTools()`, `onToolCall()`, event listeners. No subclassing or monkey-patching.
