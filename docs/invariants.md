...
`relay_outbox` and `relay_inbox` are deleted at release N+1. Startup refuses a populated legacy table and names the release-N drain-and-drop binary required before upgrade; empty leftover tables are dropped. `relay_cycles` remains local telemetry.
...
Response kinds (`stream_chunk`, `stream_end`, `result`, `error`, `status_forward`) remain durable-work rows for the requester awaiter; they must NOT enter `executeImmediate()`. The relay executor handles request kinds only.
