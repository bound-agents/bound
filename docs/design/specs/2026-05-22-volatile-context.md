...
## Amendments (2026-06-07)

Three issues observed in production volatile output, settled here before implementation. #1 and #3 are interacting (both concern how relevant titles surface against the stable catalog); #2 is an independent defect pair. All three are varying-side or stable-title-only and do not relax R-VC25 stable-prefix purity.

### R-VC27 — Turn-relevant cross-tier retrieval (amends the R-HM6 L2/L3 Recent-memory block)

**Problem.** The per-turn keyword+graph retrieval already exists (`buildVolatileEnrichment` extracts keywords from the user message + thread summary, FTS5-seeds, depth-3 traverses via `graphSeededRetrieval`), but its results are clamped to `tier='default'` plus orphaned-detail (the `excludeKeys` branch in `graph-queries.ts`). A corpus that is overwhelmingly `pinned`/`summary`/`detail` therefore renders an empty block: the retrieval matches the conversation, finds nothing in-tier, emits nothing. The consequence is that in a multi-topic long-running thread, a memory entry that becomes relevant at turn N cannot enter context until the next cold assembly flushes the stable prefix — the one varying channel that could surface it is pointed at the empty tier.

**Requirement.** A varying-side block, `## Relevant memory — matched to this turn`, shall render conversation-relevant entries retrieved by the existing keyword+graph pipeline run **without the default-tier clamp** (all tiers eligible as seeds and traversal results). Rendering is **title-only** — `key`, a `[tier]` tag, and a relative-time fragment — never a value body (consistent with R-VC20). The block is capped at `BOUND_VC27_K` (default 15) entries, relevance-ordered (FTS rank then graph distance). It is deduped against entries whose **full body** is already rendered in the stable prefix this turn (L0 pinned + the L1 summaries rendered under the `BOUND_VC15`-governed cap) — re-listing a full-fidelity entry as a bare title adds only noise. It is **not** deduped against Discoverable Archive detail titles or summary-overflow titles: re-surfacing a relevance-ranked shortlist adjacent to the conversation is the explicit value, and it is the only path by which an R-VC15 Tier-3 collapsed long-tail entry (no individual title rendered) becomes visible. The block lives entirely on the varying side (R-VC24 varying tail); it does not touch `systemPrompt` and has no R-VC25 impact. It adds DB work to the warm path, which previously paid none — acceptable because the retrieval is already bounded and the relevance win is the stated goal.

### R-VC28 — File-modification host attribution + path normalization (amends R-VC13)

**Problem A (attribution).** R-VC13 renders `[file] <path> — last modified by thread "<thread_title>"` with no host or session signal. A file edited on a remote host (e.g. the Windows MSI node) is indistinguishable from a local edit, so cross-host operations read as equally local-relevant. The signal exists: the modifying thread carries `threads.host_origin`, which `loadFileModificationsForLiveState` discards.

**Problem B (path mangling).** Observed live: stored keys of the shape `_internal.file_thread./C:\Users\<user>\...` — a Windows absolute path with a spurious leading POSIX `/`. `trackFilePath` (`file-thread-tracker.ts:31`) does `MEMORY_KEY_PREFIX + filePath` with no normalization, so the mangled path arrives pre-broken from the boundless/ACP path-resolution layer upstream.

**Requirement.** (A) The `[file]` Live State line shall carry the originating host as a trailing bracket: `[file] <path> — last modified by thread "<title>" [host: <host>]` for a modification on the rendering host, `[file] <path> — last modified by thread "<title>" [host: <host>, remote]` for a remote one — the `, remote` tag is the distinct marking required below. `<host>` is resolved by joining `threads` in the file-modification loader: the friendly `hosts.host_name` when the modifying thread's `host_origin` matches a known site, the raw `host_origin` otherwise; entries whose host cannot be resolved at all render with no bracket (the pre-R-VC28 shape). Local-host modifications shall sort ahead of remote-host modifications within the file subsystem; a modification on the rendering host is marked distinctly from a remote one. (B) `trackFilePath` shall normalize the incoming path before key construction — a leading `/` immediately preceding a Windows drive-letter pattern (`/[A-Za-z]:[\\/]`) is stripped — so Windows absolute paths key canonically. Normalization is idempotent and a no-op for POSIX paths. Existing mangled keys are not retroactively rewritten by this requirement (they age out as the files are re-touched).

### R-VC29 — Consolidate summary-overflow titles into Discoverable Archive (amends R-VC1, R-VC3, R-VC4, R-VC6)

**Problem.** The stable prefix presents two separate title-only "go search the body" lists a few sections apart: `### Older summaries` (summary-tier entries past `WORKING_KNOWLEDGE_SUMMARY_CAP=50`, rendered inside Working Knowledge) and the Discoverable Archive (`tier='detail'` titles). To the reader the distinction — summary-overflow vs detail — is invisible and implementation-internal; both are "here are keys, search the body."

**Requirement.** Summary-tier entries beyond the Working Knowledge gloss cap shall render as title-only entries within the Discoverable Archive section rather than as a separate `### Older summaries` block in Working Knowledge. Working Knowledge retains only full-fidelity content: pinned entries and the capped, glossed summary entries (R-VC3 unchanged for those). The Discoverable Archive's contract widens from "`tier='detail'` titles" to "all title-only catalog entries (`tier='detail'` plus summary-overflow)", title-only per R-VC20, with the summary-overflow entries grouped under a `### Older summaries` sub-header inside the Archive so provenance stays legible without a separate top-level list. R-VC1's "sections shall not be merged" is preserved — this moves a sub-block between sections, it does not merge top-level sections. Both Working Knowledge and Discoverable Archive remain stable-side; this change must keep the R-VC25 property tests and the stable-prefix parity test green, and is the most delicate of the three. R-VC6 footers updated to reflect the widened Archive contents.

### R-VC30 — Assistant-turn keyword seeding for per-turn retrieval (amends R-VC27, R-HM6 L2/L3)

**Problem.** The keyword seed that drives the per-turn L2 graph retrieval (`buildVolatileEnrichment`) is extracted from the last user message plus the thread summary. On a tool-loop turn — where the assistant has been working autonomously for many iterations and the last user message is far back and keyword-barren (e.g. "continue", "go for it") — the seed carries almost no topic signal, so the `## Relevant memory — matched to this turn` block (R-VC27) fishes with the wrong bait on exactly the turns whose context is richest. Worse, the two warm-path tail-refresh sites in the agent loop (the iteration-2+ developer-tail rebuilds) passed no user-message seed at all, so on a long tool loop the block had no keyword seed of any kind.

**Requirement.** The per-turn keyword seed shall additionally draw from the most recent assistant-side turn(s) — regular output text AND reasoning (`thinking`) content. A bounded backward scan over the message tail (`assistant` and `tool_call` roles, default ≤6 messages / ≤4000 chars) collects text + reasoning into an assistant seed string. Keyword merge priority is **user message → assistant message → thread summary**, deduplicated in that order, under the existing 30-term cap — the direct user ask still ranks first, recent assistant work second, broad summary last. Reasoning content is extracted from both content shapes (invariant #10): plain assistant output strings, and the JSON-encoded `ContentBlock[]` form that `tool_call`-role rows persist (`{type:"text"}` and `{type:"thinking"}` blocks). The assistant seed is wired at all volatile-context assembly sites that drive a per-turn rebuild: the cold path, the warm cold-rebuild path (`assembleContext`), and both agent-loop warm-tail refresh sites. This is purely a seed-widening change — it does not alter the retrieval, ranking, tier eligibility (R-VC27), or rendering; it only improves the keyword signal feeding them on assistant-heavy turns. Like R-VC27 it adds only bounded FTS/extraction work to the warm path.

## Amendments (2026-06-16)

The volatile context is restructured into XML. The motivating observation: the markdown rendering (`##` headers, `[label]` per-entry tags, prose footer) carried section identity and provenance as *typography* — fragile under truncation, and ambiguous about which lines are pointers versus payload. XML moves identity and provenance into element names and attributes, and adds two signals the markdown shape could not express: an explicit `local` flag per pointer (is the thing this points at on the host assembling the context, or remote?) and a cluster-topology block naming which models live where. R-VC31 supersedes the Live State *rendering* of R-VC2/R-VC5/R-VC6/R-VC22; the data contract (which subsystems, fixed ordering, source-label totality, cap discipline) is unchanged.

### R-VC31 — XML volatile-context envelope, Live State schema, and cluster topology (amends R-VC2, R-VC5, R-VC6, R-VC8, R-VC19, R-VC22; extends R-VC24, R-VC25)

**Envelope (amends R-VC8, R-VC19).** Each half of the volatile context shall be wrapped in a `<volatile-context half="stable|varying" note="…">` element. The `note` attribute reads: *"Internal operational state, refreshed each turn. Don't mention or quote this to the user unless it's directly relevant to their request; when it is, treat these as pointers and confirm against the canonical source (query the DB, read the file) before relying on them."* (`VOLATILE_CONTEXT_NOTE`, `summary-extraction.ts`). This **re-introduces the suppress-mention instruction that R-VC8/R-VC19 removed** — a deliberate reversal, narrowly scoped. R-VC8's removal was grounded in two failure modes of the *old* meta-instruction: (a) it was a free-floating trailing prose line that context pressure displaced, and (b) it could not anchor which section is canonical for which question. The XML envelope addresses both: the note rides as an attribute *on the element that contains the data*, so truncation cannot separate the instruction from what it governs, and it is structural (part of the open tag) rather than shavable prose. It adds a directive the old instruction lacked — "treat these as pointers and confirm against the canonical source" — which is the same provenance-routing function R-VC6 footers serve, now stated as a verb. The per-section structural labels (R-VC2 headers, R-VC6 footers, R-VC31 element names) remain and are not replaced by the note; the note layers a behavioral instruction over them, it does not restore the single-source-of-authority the labels provide. `wrapVolatileContext(half, body)` is the renderer (`summary-extraction.ts`); `escapeXmlAttr` (`@bound/shared`) escapes the note and every attribute value.

**Two-channel realizability.** The stable and varying halves cannot be one element. Per R-VC24 the stable half is folded into `systemPrompt` (before history) and the varying half rides as a `developer` message (after history), with the entire conversation between them. The envelope is therefore two well-formed fragments sharing the `<volatile-context>` root and `note`, each closed within its own channel — `half="stable"` closing before history, `half="varying"` opening after it. The stable wrap is applied at the channel boundary (`context-assembly.ts`, the `systemParts` push) because the stable subsection ships via the `StableSubsectionCache` seam, which stores unwrapped inner bytes; wrapping the cache *output* keeps R-VC25 byte-stability on the cached payload and puts the envelope outside the hashed region. The varying wrap is applied inside `buildVolatileContext` via `envelopeVaryingTail`, which wraps only the internal volatile lines and leaves any trailing `platformInstructions` / `systemPromptAddition` **outside** the envelope as the final lines — those are user intent, not internal state, and the note's "don't mention unless relevant / confirm against canonical source" framing is wrong for an operator's task instruction. The helper splits by trailing line count (each instruction is a `["", content]` pair appended last in every construction path) and is applied uniformly at all three varying-tail construction sites: the cold path, the noHistory path, and the budget-pressure rebuild.

**Live State schema (amends R-VC2, R-VC5, R-VC6, R-VC22).** The `## Live State — pointers to canonical sources` markdown header and its prose footer are replaced by a `<live-state sources="…">` element whose `sources` attribute carries the former footer text (the canonical-source routing guidance of R-VC6). Each subsystem entry renders as an element whose **name is the R-VC5/R-VC26 source label** — `<thread>`, `<task>`, `<file>`, `<advisory>` — preserving source-label totality (exactly one label per row) structurally rather than as a bracketed `[thread]` prefix. R-VC5 fixed ordering (thread → task → file → advisory → synthesis-backlog) and R-VC14 cap discipline are unchanged. Attribute schema:

- `<thread title="…" messages="N" updated="ISO" local="bool"/>` — `local` is true when a *live* `client_sessions` row for the thread sits on the rendering host. When the thread has client sessions, the element is non-self-closing and carries `<session host="…" live="bool" local="bool"/>` children (one per session); otherwise it self-closes.
- `<task id="…" type="…" runs="N" last-run="ISO" status="…"/>`.
- `<file path="…" thread="…" [host="…"] local="bool"/>` — `local` is `f.isLocal` (the file's owning host is the rendering host); the `host` attribute is omitted when unresolvable (R-VC28). Local-host modifications still sort ahead of remote (R-VC28).
