...
- **§12.7 (`boundctl` CLI)**: Extended. `boundcurl` is a new binary target (R-SE18). The CLI compilation in `scripts/` and `packages/cli` must be updated to produce the `dist/boundcurl` binary.
