...
4. **Soft budget vs. physical ceiling.** The recent tier may exceed the soft `historyBudget` to preserve the cache-warm anchor (an inner-loop run overshooting the soft target still fits the window). It may not exceed `recentHardCeiling` — the physical-window headroom (`effectiveBudget` minus fixed costs: system prompt + volatile tail + tools). The clamp **triggers** on the physical ceiling but **targets** the soft 65% `recentBudget`, freeing the remaining ≈35% for the middle tier to fold the shed within-turn tail (recall preserved as a compressed action log rather than dropped to ancient). When the ceiling is non-positive (fixed costs alone exceed the window), the clamp is disabled and the anchor + ≥2-message floor win. Without this distinction, a long autonomous tool-only run within one user turn re-anchored to a distant user message and pulled the entire tail into the recent tier at full resolution, defeating the budget and growing context unbounded turn over turn (observed live: ~280k tokens against a 200k window).
