...
| Restart | Durable for dispatcher-owned work (background aux): the seed rides `dispatch_queue`, bootstrap resets interrupted entries to pending, and recovery re-dispatches the thread — the errand re-runs from its durable seed. A tool that instead fires a bare in-process promise remains non-durable by construction. |
...
First consumer: the `aux` tool's `invoke` action (`packages/agent/src/tools/auxiliary.ts`). With `background: true` and a `callId` in hand it seeds the child thread — stamping `metadata.background_parent` (parent thread id + placeholder call_id + agent name) on the seed message — enqueues that seed through `dispatch_queue` (`enqueueMessage` + `notify:enqueued`), and returns a `DeferredToolResult`. The server dispatcher (`handleThread` in `packages/cli/src/commands/start/server.ts`) recognizes the correlation via `findBackgroundAuxSeed`: an aux thread WITH a correlated seed is dispatcher-owned — it runs a capability-scoped `AuxAgentLoop` built by `agentLoopFactory.createAuxLoopRunner()` (client tools resolved through the parent thread's live session) and resolves the parent's placeholder via `resolveDeferredToolResult`, guarded by `findBackgroundPlaceholderDeliveryState` — resolution stamps `background_delivered` on the row, so the guard keys on proof-of-delivery rather than on the in-flight `background` marker (which has been observed missing, #220) and a spurious re-wake can't clobber a delivered result while a stamp-less placeholder still receives its only delivery. An aux thread WITHOUT the correlation belongs to a foreground nested loop and is never claimed. Restart durability is the ordinary dispatch machinery — no bespoke journal, no scheduler task. Without a `callId` the invoke falls back to the blocking path.
...
2. History is truncated via the **progressive-fidelity "telescope" model** (`tieredHistoryTruncation` in `progressive-fidelity/tier-allocation.ts`), which replaces the binary truncation cliff with a three-tier gradient sized against a soft target of `contextWindow - maxOutputTokens` (cache-friendly headroom — the exact room the model's own response needs, computed by `computeBaseTruncationTarget` in `context-assembly.ts`):
   - **RECENT** (≈65%): full-resolution messages, backward-filled from the tail.
   - **MIDDLE** (≈30%): older messages mechanically folded into compact one-liner action summaries (`tool-cycle-fold.ts`) — deterministic projection, never LLM summarization. Each tool cycle folds to `[tool] name(arg-hint) → <result summary>`, where the summary captures the first substantive output line (e.g. `exit 0, On branch main`) rather than a bare `exit 0`, so the digest records what each command *found*, not just that it ran. `extractTextContent` unwraps tool results persisted as a JSON-serialized `ContentBlock[]` string (the boundless/MCP shape) before summarizing, so raw block JSON never leaks into the digest. The fold reads only the frozen, persisted message content — a tool's output is an immutable snapshot, so volatile-looking data (timestamps, PIDs) in a result is still byte-stable across cold rebuilds. The digest rides msg[0]'s cached prefix; a byte-stability guard in `tier-allocation.property.test.ts` pins that folded lines for the same middle-zone messages are identical as later messages append.
   - **ANCIENT** (≈5%): a fixed-size marker with the dropped-message count plus the thread summary.
3. **Recent-tier anchoring.** The recent tier is anchored to the **latest user message** — the same semantic boundary the cache-point placer (`cache-marker.ts`), `computeCompactionBoundary`, and the summary throttle all use. Within a single user turn the anchor does not move as inner-loop tool messages append, so the ancient drop count stays byte-stable and the message-level prefix cache reads back. The anchor advances by exactly one user turn when a new user message arrives — the natural cache-invalidation cadence.
4. **Soft budget vs. physical ceiling.** The recent tier may exceed the soft `historyBudget` to preserve the cache-warm anchor (an inner-loop run overshooting the soft target still fits the window). It may not exceed `recentHardCeiling` — the physical-window headroom (`effectiveBudget` minus fixed costs: system prompt + volatile tail + tools), scaled by `recentHardCeilingDeflator` so the ceiling is expressed in the same tiktoken-estimate units the tier function compares against. That deflator equals `min(1, 1 / inflationEMA)`, computed by the agent loop and passed into `assembleContext`, so on thinking-heavy threads where the estimator under-counts the real wire prompt by 1.5–2×, "recent fits the ceiling (estimated)" still implies "recent fits the window (real)"; the factor is clamped to ≤ 1 so an over-counting estimator never loosens the ceiling. The clamp **triggers** on the physical ceiling but **targets** the soft 65% `recentBudget`, freeing the remaining ≈35% for the middle tier to fold the shed within-turn tail (recall preserved as a compressed action log rather than dropped to ancient). It steps the slice start forward only up to `n − 2` (the recent tier never drops below the 2-message floor), then backs off to a wire-legal opener while that back-off still fits the ceiling — a residual leading `tool_result` opener is accepted (the driver's conversation-start invariant prepends a synthetic user message). When the ceiling is non-positive (fixed costs alone exceed the window), the clamp is disabled and the anchor + floor win. Two failure modes this guards against, both observed live on the same thread: (a) without the ceiling, a long autonomous tool-only run within one user turn re-anchored to a distant user message and pulled the entire tail into the recent tier, growing context unbounded (~280k against a 200k window); (b) without the floor guard, the clamp's tool_result-strip ran the slice start off the end on a trailing-`tool_result` tail, collapsing the recent tier to 0–1 messages and starving the agent of its own recent work (a re-derivation loop).
5. Ancient-marker and middle-digest `developer` messages are prepended ahead of the kept recent slice, and the result is returned. Property contracts (budget compliance, coverage, wire-legal opener, recency, monotonicity, determinism, graceful degradation, chronological ordering) are pinned in `progressive-fidelity/__tests__/tier-allocation.property.test.ts`.
...
- **High-water gate**: merged token estimate exceeds `truncationTargetTokens`. `truncationTargetTokens` is the per-thread adaptive target from `resolveAdaptiveTruncationTarget(db, threadId, baseTarget)`, where `baseTarget = computeBaseTruncationTarget(contextWindow, maxOutputTokens) = contextWindow - maxOutputTokens` — the exact room the upcoming model call needs to reserve for its own response. The base target is then divided by the EMA of `actualTotalTokens / totalEstimated` over the last 10 valid turns (clamped so inflation < 1.0 does not loosen the gate). `actualTotalTokens` is computed at the per-turn `applyActualUsageToContextDebug` call site as `parsed.usage.inputTokens + parsed.usage.cacheReadTokens + parsed.usage.cacheWriteTokens` — the full on-wire prompt size. `inputTokens` here is the **non-cached portion only** (see `extractUsage` in `packages/llm/src/ai-sdk-bridge.ts`, which reads `u.inputTokenDetails.noCacheTokens` and falls back to `u.inputTokens` only when details are absent), matching what AWS bills at the full input rate. The cache fields are summed back in for inflation-EMA purposes because cache reads + writes occupy wire bytes; they're discounted in pricing, not absent from the prompt. When the gate fires, the agent loop first calls `compactStoredMessagesInPlace`; only if the post-compaction estimate is still over budget does it clear `CachedTurnState` and bail to cold reassembly. The adaptive layer addresses thinking-heavy threads where `cl100k_base` under-counts by 2x+ and the raw `contextWindow - maxOutputTokens` subtraction leaves the configured forcing budget exposed. Replaces the pre-2026-07-04 ratio scheme (`TRUNCATION_TARGET_RATIO = 0.85`), which reserved an arbitrary fixed fraction of the window regardless of how large the model's actual output budget was. `COLD_COMPACTION_THRESHOLD = 500` and `computeRecentWindow(contextWindow)` are exported from `warm-compaction.ts` and shared between the cold-path Stage 1.7 entry gate (in `context-assembly.ts`) and the warm-path in-place compactor.
