...
2. History is truncated via the **progressive-fidelity "telescope" model** (`tieredHistoryTruncation` in `progressive-fidelity/tier-allocation.ts`), which replaces the binary truncation cliff with a three-tier gradient sized against a soft target of 85% of the window (cache-friendly headroom):
