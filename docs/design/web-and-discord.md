...
**Bootstrap** (`lib/mcp-apps-bootstrap.ts`, run once from `App.svelte`'s `onMount`): fetch the app-bearing server list, `connectToMcpServer` to each through its same-origin proxy (Streamable HTTP with SSE fallback), and `listTools`. The connection exists so the renderer can read the server's `ui://` app resources and route an app's `AppBridge.callServerTool` callbacks back to the server — it does **not** register the tools on the `BoundClient`. Per-server connect failures are logged and skipped so one unreachable server doesn't sink the rest. The populated `McpAppHost` is exposed via the `mcpAppHost` store for the renderer.
