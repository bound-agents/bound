...
**Config and discovery.** App-bearing servers are discovered by joining `mcp.json` (the agent's server-side connections) against the synced **capability inventory** — `hosts.mcp_capabilities`, captured at connect time, records each server's `serverInfo` and the per-tool `ui://` resource bindings — and the synced **ownership map** (`hosts.mcp_servers`, the set of server names each host configures). `GET /api/mcp-apps` (`routes/mcp-apps.ts`) returns the browser-reachable subset (`http` transport) as same-origin `proxyPath`s plus each server's UI-bearing `tools`; the real upstream `url` and any auth `headers` stay server-side and are injected by the proxy, never sent to the browser. `stdio` servers are excluded — a browser cannot reach them.

**Location transparency (R-MO27b).** The proxy is not limited to servers this host configures. Server identity is per-host — `(site_id, name)` — and each `proxyPath` carries the owning `site_id` (`/api/mcp-apps/proxy/:site/:name`), so an app-bearing server configured only on another host still renders here and the selector stays unambiguous when two hosts configure the same name. The proxy exists to dodge CORS: the browser needs a same-origin endpoint, and cross-host traffic rides the relay, the same doctrine as R-UD12 tool dispatch (any host serves any surface; affinity is an optimization). A selector that does not resolve to exactly one known `(site, name)` is rejected before any fetch. Two owner cases:

- **Local owner (this host configures the server).** The proxy fetches upstream directly and attaches the credential at this fetch edge. For a static-`headers` server the configured header is injected byte-identically; for an `auth: oauth` server the owner attaches `Authorization: Bearer <token>` sourced through its own `McpTokenStore` (with the refresh path). No live grant → the proxy returns `401` naming the pending challenge id (resolve with `bound login --mcp <server>`), never an unauthenticated upstream fetch. The token never appears in a response to the browser.
- **Peer owner (another host configures the server).** The proxy relays the MCP call to the owning host over the durable-work spool (`mcp_app_proxy` relay kind), and the OWNER resolves the upstream URL + credential from its own config/store and attaches its token at ITS edge — tokens never move (R-MO24). The relay payload carries only the JSON-RPC request body and an allow-listed MCP header subset (`accept`, `content-type`, `mcp-session-id`, `mcp-protocol-version`, `last-event-id`); a browser cookie or `Authorization` header is never forwarded. The owner ships back status + an allow-listed header subset + the base64 body over the `response:<requestId>` awaiter; the serving host replays it to the browser. The relay wait is bounded by `sync.relay.inference_timeout_ms` (default 300s), returning `504` on expiry. The browser drives Streamable HTTP request/response POST — the bootstrap opens no standalone server→client GET SSE stream (verified against `mcp-app-host.ts` `StreamableHTTPClientTransport` + `mcp-apps-bootstrap.ts` `listTools`/`callTool`), so the relay is request/response only; a POST whose response is `text/event-stream` returns its framed body verbatim.
