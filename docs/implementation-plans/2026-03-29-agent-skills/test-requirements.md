...
| Criterion | Description | Test Type | What to Verify |
|-----------|-------------|-----------|----------------|
| AC3.1 | When active skills exist, volatile context includes a `<available_skills>` block | Unit | Insert an active skill row into `skills`. Call `assembleContext(...)`. Find the system message content containing `<available_skills>`. Verify it includes `<name>{name}</name><description>{description}</description>`. |
| AC3.2 | When no active skills exist, no SKILLS block appears in volatile context | Unit | Ensure no skills exist in DB. Call `assembleContext(...)`. Verify no system message content contains `<available_skills>`. |
