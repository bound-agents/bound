...
- **15 native agent tools** with structured JSON schemas (`schedule`, `query`, `memory`, `cache`, `skill`, `advisory`, `emit`, `cancel`, `purge`, `notify`, `introspect`, `archive`, `model_hint`, `hostinfo`, `await_event`). Tools receive typed parameters directly from the LLM, eliminating argument-parsing bugs.
